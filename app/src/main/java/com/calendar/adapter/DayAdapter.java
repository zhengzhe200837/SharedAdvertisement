...
import com.calendar.bean.DateItem;
...
public class DayAdapter extends RecyclerView.Adapter<DayAdapter.ViewHolder> {
    ...
    public DayAdapter(Context context, List<DateItem> datas, int dayTextColorSelected,
                      int dayTextColorNormal, int daySelectionColor, int weekTextColor,
                      int pressShapeSelectorId, int todayPointColor) {
        this.datas = datas;
            ...
            public void onClick(View v) {
                if (itemClick != null) {
                    itemClick.onItemClick(datas.get(position).getYear(),
                            datas.get(position).getMonth(), datas.get(position).getDay(), datas.get(position).getWeek());
                }
            }
        });
    }
